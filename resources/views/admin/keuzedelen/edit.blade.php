...
<div class="max-w-2xl mx-auto">
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Keuzedeel Bewerken</h1>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form method="POST" action="/admin/keuzedelen/{{ $keuzedeel->id }}">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <textarea id="beschrijving" name="beschrijving" rows="4"
                          class="w-full border rounded px-3 py-2">{{ old('beschrijving', $keuzedeel->beschrijving) }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="periode" name="periode" class="w-full border rounded px-3 py-2" required>
                    <option value="1" {{ old('periode', $keuzedeel->periode) == '1' ? 'selected' : '' }}>Periode 1</option>
                    <option value="2" {{ old('periode', $keuzedeel->periode) == '2' ? 'selected' : '' }}>Periode 2</option>
                    <option value="3" {{ old('periode', $keuzedeel->periode) == '3' ? 'selected' : '' }}>Periode 3</option>
                    <option value="4" {{ old('periode', $keuzedeel->periode) == '4' ? 'selected' : '' }}>Periode 4</option>
                </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" id="max_studenten" name="max_studenten" 
                           value="{{ old('max_studenten', $keuzedeel->max_studenten) }}"
            ...
            <div class="mb-6 space-y-2">
                <label class="flex items-center">
                    <input type="checkbox" name="is_actief" class="mr-2" 
                           {{ old('is_actief', $keuzedeel->is_actief) ? 'checked' : '' }}>
                    <span>Actief (zichtbaar voor studenten)</span>
                </label>
                <label class="flex items-center">
                    <input type="checkbox" name="herhaalbaar" class="mr-2"
                           {{ old('herhaalbaar', $keuzedeel->herhaalbaar) ? 'checked' : '' }}>
